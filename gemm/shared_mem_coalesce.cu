...
__global__ void naive_mm(const float *input_a, const float *input_b, float *output_c, size_t m, size_t n, size_t k, int tile_w)
{
    ...
    {
        // result = tile_idx*tile_w+tile_col<k && row<m ? input_a[row*k+tile_idx*tile_w+tile_col]:0.0f;
        // As[tile_w*tile_row+tile_col];
        // __syncthreads();
        int tile_offset = tile_idx * tile_w;
        As[tile_w * tile_row + tile_col] = tile_offset + tile_col < k && row < m ? input_a[row * k + tile_offset + tile_col] : 0.0f;
        Bs[tile_w * tile_row + tile_col] = tile_offset + tile_row < k && col < n ? input_b[(tile_offset + tile_row) * n + col] : 0.0f;
        __syncthreads();
